...
function make_disp_image($folder, $src, $dest, $dis_img_width) {

    $source_image = imagecreatefromjpeg($folder . '/' . $src);
    $width = imagesx($source_image);
    $height = imagesy($source_image);

    $thumb_height = floor($height * ($dis_img_width / $width));

    $virtual_image = imagecreatetruecolor($dis_img_width, $thumb_height);

    imagecopyresampled($virtual_image, $source_image, 0, 0, 0, 0, $dis_img_width, $thumb_height, $width, $height);
...
if (isset($_GET['album'])) {
    ...
    foreach ($src_files as $file) {
        ...
        if (in_array($ext, $extensions)) {
            ...
            if (!is_dir($src_folder . '/disp_images')) {
                mkdir($src_folder . '/disp_images');
                chmod($src_folder . '/disp_images', 0777);
                //chown($src_folder.'/thumbs', 'apache'); 
            }

            $disp_image = $src_folder . '/disp_images/' . $file;
            if (!file_exists($disp_image)) {
                make_disp_image($src_folder, $file, $disp_image, $dis_img_width);
            }
        }
    }
}
...
function print_pagination($numPages, $urlVars, $currentPage) {
    ...
    if ($numPages > 1) {
        ...
        echo '<span style="font-size:small">Page ' . $currentPage . ' of ' . $numPages . '</span>';
        echo '&nbsp;&nbsp;&nbsp;';
        ...
        for ($e = 0; $e < $numPages; $e++) {
            ...
            echo '<a class="' . $class . '" href="?' . $urlVars . 'p=' . $p . '"><span style="font-size:large">' . $p . '</span></a>';
        }
...
if (!isset($_GET['album'])) {
    ...
    if (count($files) == 0) {
        ...
        echo '<div class="titlebar">
           <div class="float-left"><span class="title">' . $_GET['album'] . '</span> - <a href="' . $_SERVER['PHP_SELF'] . '"><i class ="fa fa-arrow-left" aria-hidden="true"></i> Back to Gallery</a></div>
        ...
        for ($i = $start; $i < $start + $itemsPerPage; $i++) {
            ...
            if (isset($files[$i]) && is_file($src_folder . '/' . $files[$i])) {
                ...
                echo '<div class="thumb shadow">
	                <div class="thumb-wrapper">
					<a href="' . $src_folder . '/disp_images/' . $files[$i] . '" class="albumpix" rel="albumpix">
