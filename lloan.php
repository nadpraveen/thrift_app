...
//                                $date_diff_in_moths = $interval->format('%y years and %m months');
                ...
                // FOR 50% CALUCLATION
                $loan_amount_half = ($loanamount * 0.50);
    ...
    <div class="col-md-12">
        ...
        if ($count > 0) {
            ...
            if (empty($default_msg)) {
                if ($loan_count > 0) {
                    if ($present_loan_balence < $loan_amount_half) {
                        ?>
                        ...
                        $fetch_salary_rec = new Database;
                        $fetch_salary_rec->prepare("SELECT sum(AMOUNTP) as total_sal_rec FROM th_loan_trans where LOAN_NO = $loan_number and MODE_OF_PAYMENT = 'S'");
                        $sal_rec = $fetch_salary_rec->resultset();
                        foreach ($sal_rec as $sal_rec) {
                            $sal_rec_sum = $sal_rec['total_sal_rec'];
                        }
                        if ($eligble_amount1_25 > $sal_rec_sum) {
                            $amount_to_pay = $eligble_amount1_25 - $sal_rec_sum;
                            ?>
                            <div class="alert alert-info">
                                You are eligible for next loan after repaying of Rs. <?php echo $amount_to_pay ?>/- Through Salary
                            </div>
                            <?php
                        }
                    }
